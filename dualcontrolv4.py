...
#
# Version 75 (Faster Data Logging):
#   - Reduced the data logging period at each setpoint from 10 seconds
#     to 5 seconds to increase the overall speed of the calibration process.
#
# Version 74 (Moving Average Learning):
#   - Upgraded the learning mechanism to be more robust against outliers.
#   - The system now stores a history of the last 10 successful runs for
#     each setpoint within 'learned_outlet_positions.json'.
#   - Predictions for the outlet valve are now based on a moving average
#     of this historical data, providing smoother and more reliable
#     performance while still adapting to long-term system changes.
# ==============================================================================
...
class StateMachinePressureController:
    ...
    def set_pressure(self, pressure, predicted_outlet_pos=None):
        ...
        if pressure == 0:
            ...
            if self.previous_setpoint == 0 and predicted_outlet_pos is None:
        ...
        try:
            ...
            for sp in setpoints:
                ...
                if current_outlet_pos is not None:
                    ...
                    self.log_message(f"  Updated history for {sp:.2f} Torr. Now have {len(self.learned_outlet_positions[sp_key])} data point(s).")
            ...
            if self.is_calibrating:
                ...
                try:
                    ...
                    self.log_message(f"Saved {len(self.learned_outlet_positions)} learned points for future use.")
