# ADS1256.py - V5 Final with robust negative voltage handling

import config
import RPi.GPIO as GPIO
import time
import ctypes # Import ctypes library for robust signed integer conversion

ScanMode = 0

# (omitting gain, data rate, register, and command dictionaries for brevity - they remain unchanged)
# gain channel
...
class ADS1256:
    ...
    def ADS1256_ConfigADC(self, gain, drate):
        self.ADS1256_WaitDRDY()
        buf = [0,0,0,0,0,0,0,0]
        buf[0] = (0<<3) | (1<<2) | (0<<1) # Enable Buffer
        buf[1] = 0x08
    ...
    def ADS1256_SetDiffChannal(self, Channal):
        if Channal == 0:
            self.ADS1256_WriteReg(REG_E['REG_MUX'], (0 << 4) | 1) 	# AIN0-AIN1
        elif Channal == 1:
            self.ADS1256_WriteReg(REG_E['REG_MUX'], (2 << 4) | 3) 	# AIN2-AIN3
        elif Channal == 2:
            self.ADS1256_WriteReg(REG_E['REG_MUX'], (4 << 4) | 5) 	# AIN4-AIN5
        elif Channal == 3:
            self.ADS1256_WriteReg(REG_E['REG_MUX'], (6 << 4) | 7) 	# AIN6-AIN7
    ...
    def ADS1256_Read_ADC_Data(self):
        ...
        # --- KEY FIX FOR NEGATIVE VOLTAGES ---
        # This correctly converts the 24-bit two's complement value to a signed 32-bit integer
        if (read & 0x800000):
            read = -((read ^ 0xffffff) + 1)
            
        return read
 
    def ADS1256_GetChannalValue(self, Channel):
        global ScanMode
        if(ScanMode == 0): # Single-ended
    ...
    def ADS1256_GetAll(self):
        global ScanMode
        if(ScanMode == 1): # Differential mode
            num_channels = 4
            ADC_Value = [0]*num_channels
