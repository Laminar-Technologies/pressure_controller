...
#
# Version Update (Ramping Logic):
#   - Pump-to-zero ramp now triggers at >75% FS.
#   - Ramp profile adjusted to 25% over 10s, with a 5s hold period.
# ==============================================================================
        ...
        while not self._stop_event.is_set() and not self.e_stop_event.is_set():
            ...
            try:
                ...
                # If inlet is nearly closed but pressure is high, open outlet to release pressure.
                    ...
                    elif inlet_valve_pos > 95.0 and self.system_setpoint > 0 and self.previous_setpoint != 0:
                        new_outlet_pos = current_outlet_pos + step_size
    ...
    def set_pressure(self, pressure, predicted_outlet_pos=None):
        ...
        if pressure == 0:
            ...
            if current_p is not None and current_p > (self.full_scale_pressure * 0.75):
                ...
                # Ramp from 0% to 25% over 10 seconds (2.5% per second)
                for i in range(10):
                ...
                if not self.e_stop_event.is_set():
                    self.log_queue.put("Ramp complete. Holding at 25% for 5 seconds.")
                    
                    # Hold at 25% for 5 seconds, checking for E-Stop each second.
                    for _ in range(5):
                        if self.e_stop_event.is_set():
