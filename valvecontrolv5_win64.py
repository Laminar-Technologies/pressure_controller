...
#
# Version 17 (Software Scaling Fix):
#   - Adjusted pressure calculation to use a software scaling factor.
#   - This treats the clipped 9.9V signal (caused by the 20k/10k voltage
#     divider and 3.3V ADC limit) as the 100% full-scale value.
# ==============================================================================
...
class CalibrationGUI(tk.Tk):
    def __init__(self):
        ...
        self.dut_colors = ['#2ca02c', '#d62728', '#9467bd', '#8c564b'] # Green, Red, Purple, Brown
        ...
        self.manual_trace_queue = queue.Queue()
    ...
    def periodic_update(self):
        ...
        if self.is_in_manual_mode:
            ...
            if trace_data_processed:
                if len(self.manual_trace_time) > 100:
                ...
                self.ax_manual_trace.autoscale_view(scaley=True)
        ...
        if processed_data:
            ...
            if self.live_std_pressure_history:
                visible_pressures.extend(p for p in self.live_std_pressure_history if p is not None)

            for i, line in self.live_dut_plots.items():
                if line.get_visible():
                    visible_pressures.extend(p for p in self.live_dut_pressure_history[i] if p is not None and not np.isnan(p))

            if visible_pressures:
                min_p, max_p = min(visible_pressures), max(visible_pressures)
                p_range = max(max_p - min_p, 0.01)
                padding = p_range * 0.1
                self.ax_live_pressure.set_ylim(min_p - padding, max_p + padding)
            else:
                self.ax_live_pressure.relim()
                self.ax_live_pressure.autoscale_view(scalex=False, scaley=True)
    ...
    def setup_manual_display(self):
        ...
        self.manual_frame.columnconfigure(1, weight=3)
    ...
    def on_manual_focus_change(self):
        ...
        if focus_id == 'std':
            ...
            if hasattr(self, 'manual_plot_panel'):
                self.manual_plot_panel.grid_remove()
        else:
        ...
        while self.is_in_manual_mode:
            ...
            for dut in self.active_duts:
                ...
                if voltage is not None:
                    # --- SOFTWARE SCALING FIX ---
                    # Calculate pressure using the ACTUAL measured maximum (9.9V) as the 100% FS mark
                    device_pressure = voltage * (fs / 9.9)
            ...
            for sp in setpoints:
                ...
                while self.is_calibrating:
                    if (time.time() - last_live_poll_time) > 0.2:
                        ...
                        for dut in self.active_duts:
                             ch, fs = dut['channel'], dut['fs']
                             voltage = self.daq.read_voltage(dut['channel'])
                             if voltage is not None:
                                # --- SOFTWARE SCALING FIX ---
                                device_pressure = voltage * (fs / 9.9)
                                live_dut_pressures[ch] = device_pressure
                        ...
                        if is_stable:
                            ...
                            if is_in_tolerance:
                                ...
                                if out_of_tolerance_start_time is None:
                                    ...
                                    self.log_message("  10-second wait is over. Prompting user...")
                                    should_proceed = messagebox.askyesno("Out-of-Tolerance Override", 
                                        f"The pressure has been stable at {stable_pressure:.4f} Torr for over 10 seconds, but this is outside the setpoint tolerance ({sp:.4f} +/- {priority_tolerance:.4f} Torr).\n\n"
                                        "Do you want to accept this reading and proceed?")
                                    
                                    if should_proceed:
                                        self.log_message("  User accepted the out-of-tolerance reading.")
                                        break
                                    else:
                                        self.log_message("  User chose to continue waiting. Resetting 10s timer.")
                                        out_of_tolerance_start_time = None
                        else:
                            stability_confirmed_time = None
                            out_of_tolerance_start_time = None
                    ...
                    if sp in dut_specific_setpoints[ch]:
                        ...
                        if not np.isnan(mean_dut):
                            ...
                            if abs(error) > tolerance:
                                warning_msg = (f"  ⚠️ WARNING: Device {ch+1} is OUTSIDE tolerance! | "
                                               f"Error: {error:+.4f} Torr | Tolerance: +/- {tolerance:.4f} Torr")
