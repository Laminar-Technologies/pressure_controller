...
#
# Version Update (Raspberry Pi Standalone & UI/UX Improvments):
#   - Modified to run exclusively on a Raspberry Pi.
#   - DAQController is now instantiated directly instead of connecting over a network.
#   - Removed DAQ IP address configuration from the GUI.
#   - Integrated MultiplexerController to manage a 274B Multiplexer and 270B
#     Signal Conditioner via Raspberry Pi GPIO.
#   - UI layout and sizing adjusted for better readability and usability.
#   - Serial port selection now correctly lists /dev/tty devices for Linux.
#   - Corrected vertical stretching of UI elements in fullscreen mode.
#   - Reworked top panel grid geometry for robust resizing.
#   - System FS is now automatically set based on Standard selection.
# ==============================================================================
...
class CalibrationGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        # Configure main window grid: row 1 (plots) expands vertically
        ...
        # --- Configure your standard transducer ranges here ---
        # IMPORTANT: Update these values to match your hardware setup
        self.standard_fs_ranges = {
            1: 0.1,   # Channel 1 is a 0.1 Torr standard
            2: 10.0,  # Channel 2 is a 10 Torr standard
            3: 1.0    # Channel 3 is a 1 Torr standard
        }
        ...
        # --- Attributes for Main Screen Focus Feature ---
    ...
    def setup_ui(self):
        ...
        # Devices Under Test (DUTs) (Stays in Row 1, Column 1)
    ...
    def on_standard_change(self):
        ...
        standard_fs = self.standard_fs_ranges.get(channel, 1.0) # Default to 1.0 if not found
    ...
    def _connect_thread(self):
        try:
            ...
            # --- MODIFIED: FS value is now set by on_standard_change ---
            self.on_standard_change() # Ensure fs value is set before controller init
            ...
            self.log_queue.put(f"Connected to Controllers on {inlet_port} & {outlet_port}.")

            # --- MODIFIED: Instantiate DAQController directly for local use ---
            self.daq = DAQController(log_queue=self.log_queue)
            self.log_queue.put(f"Initialized local DAQ controller.")
