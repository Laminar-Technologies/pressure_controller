...
# Prerequisites:
#   - Python 3
#   - pyserial, pandas, matplotlib, numpy libraries
...
class DAQController:
    ...
    def read_voltage(self, channel):
        """Commands the DAQ to read a specific channel."""
...
class CalibrationGUI(tk.Tk):
    ...
    def setup_ui(self):
        ...
        for i in range(4):
            ...
            enabled_var = tk.BooleanVar(self, value=(i==0))
    ...
    def connect_instruments(self):
        try:
            ...
            self.start_button.config(state=tk.DISABLED); self.e_stop_button.config(state=tk.NORMAL)
    ...
    def setup_manual_display(self):
        ...
        self.manual_frame.grid(row=0, column=0, sticky="nsew", padx=10, pady=10)
        
        tk.Label(self.manual_frame, text="Manual Calibration & Adjustment", font=("Helvetica", 16, "bold")).pack(pady=5)
        
        control_frame = tk.LabelFrame(self.manual_frame, text="Setpoint Control", padx=10, pady=10)
        ...
        tk.Button(button_frame, text="Set 0% FS (Zero)", command=lambda: self.set_manual_pressure(0)).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="Set 50% FS (Linearity)", command=lambda: self.set_manual_pressure(0.5)).pack(side=tk.LEFT, padx=5)
        tk.Button(button_frame, text="Set 100% FS (Span)", command=lambda: self.set_manual_pressure(1.0)).pack(side=tk.LEFT, padx=5)

        self.manual_labels = {}
        self.manual_focus_device.set("std")

        std_frame = tk.Frame(self.manual_frame)
        std_frame.pack(fill='x', padx=10, pady=2)
        tk.Radiobutton(std_frame, text=f"Standard ({self.standard_fs_value} Torr FS)", variable=self.manual_focus_device, 
                        value="std", command=self.on_manual_focus_change).pack(side=tk.LEFT)

        for dut in self.active_duts:
            ch, fs = dut['channel'], dut['fs']
            frame = tk.LabelFrame(self.manual_frame, text=f"Device {ch+1}", padx=10, pady=5)
            frame.pack(fill="x", expand=True, padx=10, pady=5)
            
            tk.Radiobutton(frame, text=f"Focus ({fs} Torr FS)", variable=self.manual_focus_device, 
                           value=f"ch{ch}", command=self.on_manual_focus_change).pack(side=tk.LEFT, padx=5)
            
            diff_var = tk.StringVar(value="Difference: -- Torr")
    ...
    def on_manual_focus_change(self):
        ...
        if focus_id == 'std':
            self.log_message(f"Manual control focus set to Standard ({self.standard_fs_value} Torr).")
        else:
            ch = int(focus_id.replace('ch',''))
            for dut in self.active_duts:
                if dut['channel'] == ch:
                    self.log_message(f"Manual control focus set to DUT {ch+1} ({dut['fs']} Torr).")
                    break
    ...
    def update_manual_display(self, data):
        for ch, values in data.items():
            diff = values['diff']
            self.manual_labels[ch]['diff_var'].set(f"Difference: {diff:+.3f} Torr")
    ...
    def run_calibration(self):
        ...
        try:
            ...
            for sp in setpoints:
                ...
                last_live_poll_time = 0
                
                while self.is_calibrating:
                    pressure = self.baratron.get_pressure()
                    if pressure is not None:
                        if abs(pressure - sp) <= (self.standard_fs_value * 0.0005):
                            if start_stable_time is None: start_stable_time = time.time()
                            if (time.time() - start_stable_time) >= 5.0: 
                                self.log_message("  Pressure is stable.")
                                break
                        else:
                            start_stable_time = None
                    ...
                    if (time.time() - last_live_poll_time) > 0.2:
                        ...
                        for dut in self.active_duts:
                            ...
                            if voltage is not None:
                                live_dut_pressures[ch] = voltage * (fs / 10.0)
                ...
                standard_pressure = self.baratron.get_pressure()
                if standard_pressure is None:
                    self.log_message(f"ERROR: Failed to read Standard for setpoint {sp}."); continue
                
                self.data_storage['Setpoint_Torr'].append(sp)
                self.data_storage['Standard_Pressure_Torr'].append(standard_pressure)
                log_line = f"  Setpoint: {sp:.2f} | Standard: {standard_pressure:.3f} Torr"

                for dut in self.active_duts:
                    ch, fs = dut['channel'], dut['fs']
                    
                    if sp in dut_specific_setpoints[ch]:
                        daq_voltage = self.daq.read_voltage(ch)
                        if daq_voltage is not None:
                            device_pressure = daq_voltage * (fs / 10.0)
                            self.data_storage[f'Device_{ch+1}_Pressure_Torr'].append(device_pressure)
                            log_line += f" | Dev {ch+1}: {device_pressure:.3f} Torr"

                            tolerance = fs * 0.005
                            error = device_pressure - standard_pressure
                            
                            if abs(error) > tolerance:
                                warning_msg = (
                                    f"  [WARNING] DUT {ch+1} out of tolerance at {sp} Torr setpoint.\n"
                                    f"            > Error: {error:+.4f} Torr (Limit: +/- {tolerance:.4f} Torr)"
                                )
                                self.log_message(warning_msg)
                                self.error_tracker[ch].append({'std': standard_pressure, 'error': error})
                        else:
            ...
            self.is_calibrating = False
            if self.baratron: self.baratron.close_valve()
            self.after(10, self.analyze_and_suggest_tuning) # Run analysis after cal
    ...
    def analyze_and_suggest_tuning(self):
        self.log_message("\n--- Post-Calibration Tuning Analysis ---")
        any_suggestions = False
        for dut in self.active_duts:
            ch, fs = dut['channel'], dut['fs']
            errors = self.error_tracker.get(ch, [])
            ...
            # Segment errors by pressure range
            zero_errors = [e['error'] for e in errors if (e['std'] / fs) <= 0.2]
            mid_errors = [e['error'] for e in errors if 0.4 < (e['std'] / fs) <= 0.6]
            span_errors = [e['error'] for e in errors if (e['std'] / fs) >= 0.8]

            avg_zero = np.mean(zero_errors) if zero_errors else 0.0
            avg_mid = np.mean(mid_errors) if mid_errors else 0.0
            avg_span = np.mean(span_errors) if span_errors else 0.0

            # Heuristics for providing feedback
            zero_is_sig = abs(avg_zero) > (fs * 0.001)
            span_is_sig = abs(avg_span - avg_zero) > (fs * 0.002) # Check for slope change
            lin_is_sig = abs(avg_mid - (avg_zero + avg_span)/2) > (fs * 0.002) # Check for bowing

            self.log_message(f"  Avg Error @ Zero: {avg_zero:+.4f} | Mid: {avg_mid:+.4f} | Span: {avg_span:+.4f}")

            if not (zero_is_sig or span_is_sig or lin_is_sig):
                self.log_message("  Errors detected, but no clear pattern for Zero/Span/Linearity adjustment found.")
                continue

            self.log_message("  Recommended Tuning Order:")
            self.log_message("  1. ZERO: Set pressure to 0% FS. Adjust the Zero pot.")
            if zero_is_sig:
                if avg_zero > 0: self.log_message("     > Your device reads HIGH at zero. Adjust to lower the reading.")
                else: self.log_message("     > Your device reads LOW at zero. Adjust to raise the reading.")
            else:
                 self.log_message("     > Zero appears to be close. Check and confirm.")
            
            self.log_message("\n  2. SPAN: Set pressure to 100% FS. Adjust the Span pot.")
            if span_is_sig:
                 if avg_span > avg_zero: self.log_message("     > Your device reads progressively HIGHER (gain is too high). Adjust to lower the reading at 100%.")
                 else: self.log_message("     > Your device reads progressively LOWER (gain is too low). Adjust to raise the reading at 100%.")
            else:
                 self.log_message("     > Span appears to be close. Check and confirm.")
            self.log_message("     (Note: Adjusting Span may affect Zero. Re-check Zero after.)")

            self.log_message("\n  3. LINEARITY: Set pressure to 50% FS. Adjust the Linearity pot.")
            if lin_is_sig:
                 if avg_mid > (avg_zero + avg_span)/2: self.log_message("     > Mid-range reads HIGH ('smiling' curve). Adjust linearity pot.")
                 else: self.log_message("     > Mid-range reads LOW ('frowning' curve). Adjust linearity pot.")
            else:
                self.log_message("     > Linearity appears to be close. Check and confirm.")
            self.log_message("     (Note: Adjusting Linearity may slightly affect Zero and Span.)")

        if not any_suggestions:
            self.log_message("  All devices passed within tolerance or had too few errors for analysis.")
    ...
    def update_cal_plot(self):
        ...
        for dut in self.active_duts:
            valid_points = {
                'std': [s for s, d in zip(self.data_storage['Standard_Pressure_Torr'], self.data_storage[f'Device_{dut["channel"]+1}_Pressure_Torr']) if not np.isnan(d)],
                'dut': [d for d in self.data_storage[f'Device_{dut["channel"]+1}_Pressure_Torr'] if not np.isnan(d)]
            }
            self.ax_cal.plot(valid_points['std'], valid_points['dut'], 'o-', label=f'Device {dut["channel"]+1}')
