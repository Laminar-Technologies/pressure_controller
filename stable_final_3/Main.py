...
#
# Version 131 (Final UI Logic & Safety):
#   - Final correction for turbo indicator light logic to match documentation.
#   - Flashing/solid/off states for "at speed" light are now accurate.
# ==============================================================================
...
class CalibrationGUI(tk.Tk):
    ...
    def setup_ui(self):
        ...
        tk.Label(config_frame, text="DAQ:").grid(row=3, column=0, sticky="e", pady=2)
        ...
        self.daq_com_combo.grid(row=3, column=1, sticky="w", pady=2)
        ...
        for i in range(4):
            ...
            check.grid(row=i*2, column=1, pady=(5,0))
        ...
        pressure_readout_frame.grid_columnconfigure(0, weight=1)

        tk.Label(pressure_readout_frame, textvariable=self.live_pressure_var, font=("Helvetica", 22, "bold")).grid(row=0, column=0, sticky="ew")
        
        custom_sp_frame = tk.Frame(pressure_readout_frame)
        custom_sp_frame.grid(row=1, column=0, pady=5)
        ...
        system_action_frame.grid(row=2, column=0, pady=5)
