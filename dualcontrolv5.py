...
#
# Version 84 (Production Candidate):
#   - Implemented fully automatic background learning in manual mode. The system
#     now detects stable setpoints and learns them without user interaction.
#   - A terminal message now explicitly announces when stability is achieved.
#   - Extended the post-move-from-zero delay to 8 seconds for added stability.
#   - All features are now integrated for a robust, intelligent, and
    ...
    def set_pressure(self, pressure, predicted_outlet_pos=None):
        ...
        if pressure == 0:
            ...
            if self.previous_setpoint == 0:
                self.log_queue.put(">> Allowing system to settle after initial move from zero...")
                time.sleep(8.0)
            elif outlet_was_moved:
                self.log_queue.put(">> Waiting for outlet valve to physically move...")
                time.sleep(2.0)
