...
class ComprehensiveValveTesterGUI(tk.Tk):
    ...
    def setup_ui(self):
        ...
        ttk.Label(connection_frame, text="COM Port:").pack(pady=2)
        self.com_port_var = tk.StringVar()
        com_ports = [port.device for port in serial.tools.list_ports.comports()]
        self.com_port_combo = ttk.Combobox(connection_frame, textvariable=self.com_port_var, values=com_ports, state="readonly", width=10)
    ...
    def toggle_connection(self):
        if self.valve_controller and self.valve_controller.is_connected:
            ...
            if not port:
                self.log_message("ERROR: No COM port selected.")
                return
