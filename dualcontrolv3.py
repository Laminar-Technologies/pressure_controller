...
#
# Version 71 (Timestamp Logging):
#   - Added timestamps to all terminal log messages, synchronized with the
#     live pressure trace time.
#   - Captured the timestamp at the moment each setpoint is logged during
#     automated calibration.
#   - Added timestamp annotations to the DUT deviation graph, showing when
#     each error bar was logged.
#
# Version 70 (Production Ready - Intelligent Cooldown):
#   - Reverted to the superior Version 69 control logic based on user testing.
#   - Enhanced the adaptive outlet loop with an intelligent "cooldown" and
#     "leak-up recovery" mode. After an oscillation is corrected, the system
#     enters a brief cooldown to prevent overreaction.
#   - If pressure is high while the inlet is fully closed, the script now
#     correctly identifies that the outlet is too closed and gently opens it.
#     This resolves edge cases where the system could get stuck.
# ==============================================================================
        ...
        while not self._stop_event.is_set():
            ...
            try:
                ...
                if is_inlet_oscillating:
                    ...
                    # NEW: Leak-up recovery logic. If inlet is closed but pressure is high,
                    # it means the outlet is too closed. Gently open it to compensate.
                    self.oscillation_cooldown = False
                    new_outlet_pos = current_outlet_pos + 0.2
...
class CalibrationGUI(tk.Tk):
    ...
    def e_stop_action(self):
        if self.is_calibrating or self.is_in_manual_mode:
            ...
            if was_calibrating and self.debug_full_time:
    ...
    def periodic_update(self):
        # Calculate the time for this update cycle first for consistent timestamping.
        ...
        if self.state_controller and self.state_controller.is_connected:
            # 'current_time' is already calculated above.
            ...
            for i in range(4):
                ...
                if is_active:
                    ...
                    if self.is_calibrating:
                        self.debug_full_dut_pressure[i].append(np.nan)
    ...
    def run_calibration(self):
        ...
        try:
            ...
            for sp in setpoints:
                ...
                # Capture the time at the moment of logging completion.
