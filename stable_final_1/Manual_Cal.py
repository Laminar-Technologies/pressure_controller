...
class ManualCalFrame(tk.Frame):
    ...
    def setup_ui(self):
        ...
        self.manual_quad_fig, self.main_app.axs_manual_quad = plt.subplots(2, 2, figsize=(9, 7), sharex=True, sharey=False)
        self.main_app.manual_quadrant_lines = []
        for i, ax in enumerate(self.main_app.axs_manual_quad.flat):
            ...
            self.main_app.manual_quadrant_lines.append({'std': std_line, 'dut': dut_line, 'ax': ax})
        self.manual_quad_fig.tight_layout()
        self.main_app.manual_quad_canvas = FigureCanvasTkAgg(self.manual_quad_fig, master=self.manual_plot_panel)

        # Create Single plot for individual DUT focus
        self.manual_single_fig, self.main_app.ax_manual_single = plt.subplots(figsize=(9, 7))
        std_line, = self.main_app.ax_manual_single.plot([], [], 'b-', label='Standard', linewidth=2)
        dut_line, = self.main_app.ax_manual_single.plot([], [], 'g-', label='Focused DUT')
        self.main_app.manual_single_lines = {'std': std_line, 'dut': dut_line}
        self.main_app.ax_manual_single.grid(True)
        self.main_app.ax_manual_single.legend()
        self.main_app.manual_single_canvas = FigureCanvasTkAgg(self.manual_single_fig, master=self.manual_plot_panel)

        self.on_manual_focus_change()
        self.set_manual_pressure(0) # Start at zero pressure.
    ...
    def on_manual_focus_change(self):
        ...
        if focus_id == 'std':
            ...
            self.main_app.manual_single_canvas.get_tk_widget().pack_forget()
            self.main_app.manual_quad_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

            # Only show plots for active DUTs.
            for i, plot_data in enumerate(self.main_app.manual_quadrant_lines):
                is_active = any(d['channel'] == i for d in self.main_app.active_duts)
                plot_data['ax'].set_visible(is_active)
            self.main_app.manual_quad_canvas.draw_idle()
        else:
            ...
            self.main_app.manual_quad_canvas.get_tk_widget().pack_forget()
            self.main_app.manual_single_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

            # Configure the single plot for the selected DUT.
            self.main_app.ax_manual_single.set_title(f"Live Trace: DUT {ch+1} vs. Standard")
            self.main_app.manual_single_lines['dut'].set_color(self.main_app.dut_colors[ch])
            self.main_app.manual_single_lines['dut'].set_label(f'DUT {ch+1}')
            self.main_app.ax_manual_single.legend()
            self.main_app.manual_single_canvas.draw_idle()
